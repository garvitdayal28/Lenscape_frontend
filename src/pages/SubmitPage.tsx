  ...
  if (!token) {
    return (
      <div className="min-h-screen bg-exhibition-void text-exhibition-bone flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-chic-bg border border-exhibition-gold/20 p-10 text-center relative">
          ...
          <p className="text-xs font-mono text-chic-muted mb-8 leading-relaxed">Please sign in to upload artworks.</p>
          <Link to="/auth/signup">
    ...
    return (
      <div className="min-h-screen bg-exhibition-void text-exhibition-bone flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <p className="font-mono text-[10px] text-chic-muted uppercase tracking-widest">Sent to the curation committee for review</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-exhibition-void text-exhibition-bone pb-32">
      ...
      <div className="max-w-6xl mx-auto px-6 pt-24">
        ...
        <div className="text-center lg:text-left mb-12">
          ...
          <p className="text-xs font-mono text-chic-muted mt-2 max-w-sm">Configure your canvas below. All uploads are moderated.</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          {/* ── Form ── */}
          <div className="lg:col-span-7 bg-chic-light/20 border border-chic-muted/30 p-8">
            <form onSubmit={handleFormSubmit} className="space-y-6">

              {/* Title */}
              <div>
                <label className="block font-mono text-[9px] uppercase tracking-widest text-chic-primary mb-2">Canvas Title</label>
              ...
              {isVideoCategory ? (
                ...
                <div>
                  <label className="block font-mono text-[9px] uppercase tracking-widest text-chic-primary mb-2">Cover Image</label>
                  {!imagePreview ? (
                    <label className="flex flex-col items-center justify-center w-full h-44 border-2 border-dashed border-chic-muted/40 cursor-pointer hover:border-exhibition-gold/50 transition-colors">
                      <UploadCloud className="w-8 h-8 text-zinc-600 mb-3" />
                      <span className="font-mono text-[10px] text-chic-muted uppercase tracking-widest">Click to upload</span>
                      <span className="font-mono text-[8px] text-zinc-600 mt-1">PNG, JPG, GIF · max 10MB</span>
                      <input type="file" accept="image/*" className="hidden" onChange={handleFileChange} />
                    </label>
                  ) : (
                    <div className="relative w-full h-44 border border-chic-muted/40 overflow-hidden">
                      <img src={imagePreview} alt="preview" className="w-full h-full object-cover" />
                      {uploading && (
                        <div className="absolute inset-0 bg-chic-bg/70 flex items-center justify-center">
                      ...
                      <button type="button" onClick={clearImage}
                        className="absolute top-2 right-2 w-7 h-7 bg-chic-bg/80 border border-chic-muted/50 flex items-center justify-center text-chic-primary hover:text-chic-primary">
                        <X size={14} />
                      </button>
                    </div>
                  )}
              ...
              <div>
                <label className="block font-mono text-[9px] uppercase tracking-widest text-chic-primary mb-3">
                  Orientation &amp; Dimension
                </label>
                    ...
                    <button key={o.id} type="button" onClick={() => setOrientation(o.id)}
                      className={`flex flex-col items-center gap-1.5 py-3 px-1 border transition-all ${
                        orientation === o.id
                          ...
                          : 'border-chic-muted/40 text-chic-muted hover:border-chic-muted/50'
                      }`}>
                      {/* Mini ratio preview */}
              ...
              {/* Category */}
              <div>
                <label className="block font-mono text-[9px] uppercase tracking-widest text-chic-primary mb-3">Category</label>
                  ...
                  {EVENT_CATEGORIES.map(cat => (
                    <div key={cat.id} onClick={() => handleCategoryChange(cat.id)}
                      className={`border p-3 cursor-pointer text-center transition-all ${
                        category === cat.id ? 'border-exhibition-gold bg-exhibition-gold/5 text-exhibition-gold' : 'border-chic-muted/40 text-chic-muted hover:border-chic-muted/50'
                      }`}>
                      <div className="font-mono text-[9px] uppercase tracking-wider text-chic-muted">Room {cat.room}</div>
                      <div className="text-[11px] font-mono uppercase mt-1 font-bold">{cat.label}</div>
                    </div>
                  ))}
                </div>

                <label className="block font-mono text-[9px] uppercase tracking-widest text-chic-primary mb-2">Subcategory</label>
                  ...
                  {selectedCat.subcategories.map(sub => (
                    <div key={sub} onClick={() => setSubcategory(sub)}
                      className={`border px-3 py-2.5 cursor-pointer text-center transition-all ${
                        subcategory === sub ? 'border-exhibition-gold bg-exhibition-gold/5 text-exhibition-gold' : 'border-chic-muted/40 text-chic-muted hover:border-chic-muted/50'
                      }`}>
              ...
              {/* Description */}
              <div>
                <label className="block font-mono text-[9px] uppercase tracking-widest text-chic-primary mb-2">Conceptual Statement</label>
                <textarea rows={4} placeholder="The story behind your canvas..." value={description} onChange={e => setDescription(e.target.value)} required
                  className="w-full bg-white border border-chic-muted/40 text-xs font-sans px-4 py-3 text-exhibition-bone focus:outline-none focus:border-exhibition-gold/50 resize-none" />
              </div>
          ...
          <div className="lg:col-span-5">
            <h3 className="font-mono text-[10px] text-chic-muted uppercase tracking-widest mb-4">Corridor Placard Preview</h3>
            <div className="border border-chic-muted/30 bg-chic-bg/10 p-6">
              ...
              <div className={`museum-frame relative overflow-hidden mb-4 w-full ${selectedOrient.css}`}>
                {imagePreview ? (
                  <img src={imagePreview} alt="preview" className="w-full h-full object-contain bg-chic-bg" />
                ) : (
                ...
                {/* Ratio tag */}
                <div className="absolute bottom-2 right-2 bg-chic-bg/80 border border-chic-muted/50 px-2 py-0.5 font-mono text-[7px] text-chic-primary uppercase">
                  {selectedOrient.ratio.replace('/', ':')}
                </div>
              </div>
              <div className="bg-chic-bg border border-exhibition-gold/20 p-4 text-center">
                <h4 className="editorial-text text-xl font-bold text-exhibition-bone">{title || 'Untitled Canvas'}</h4>
                <p className="text-[9px] font-mono text-chic-muted uppercase tracking-widest mt-1">
                  {category.replace('-', ' ')} · {selectedOrient.label}
                </p>
              </div>
            </div>

            <div className="border border-chic-muted/30/60 p-5 mt-6 bg-chic-light/10">
              ...
              <ul className="space-y-2 font-mono text-[9px] text-chic-muted leading-relaxed list-disc pl-4 uppercase">
